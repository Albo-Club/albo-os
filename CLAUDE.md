...
- **AI** : `@convex-dev/agent` backend (default model `claude-haiku-4-5`, override via `ANTHROPIC_MODEL`) + `@assistant-ui/react` front + streaming HTTP route `/api/chat`. Provider abstracted via `getModel()` in `convex/agent.ts`. L'agent n'a **aucun outil d'action** pour l'instant (`tools: {}`) — les outils DB scopés à l'org (deals/companies) seront recâblés en V0 (`convex/agentTools.ts` à recréer, scope `${orgId}:${userId}` du thread).
