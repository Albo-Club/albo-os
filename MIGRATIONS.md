...
| Correction one-off org d'un deal en TS (DOKA)                        | `convex/migrations/reassignDealOrg.ts` → `reassignDealOrg:run`                                              | Re-parente un deal **`pending`** vers une autre org (`{"attioDealId":"…","orgSlug":"calte"}`). Déplace l'investisseur `group_root`, la ligne `forecastEntries` liée, et le stub cible **si** c'est un stub de sync (`kind:'portfolio'` + `attioCompanyId`) exclusif à ce deal (sinon stub neuf). Refuse un deal non-`pending`. Idempotent (no-op si déjà dans l'org). Le sync Attio ne fixe l'org qu'à la création → une correction `albo_or_calte` ne repropage pas. Cf. `KNOWN_ISSUES.md` « Sync Attio → deals ». |
