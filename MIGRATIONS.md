...
| Import documentation juridique Albo Club (Drive → `documents`)         | `convex/migrations/legalDocsImport.ts` → `dryRun` / `verify` + `node scripts/import-legal-docs.mjs` | Importe 320 documents sur 42 sociétés de l'org `albo` depuis le Drive « ⚠️ Investissements ». Correspondance figée et revue à la main dans `scripts/data/legal-docs-albo.json`. **Idempotent** sur `companyId`+`title`+`size` : re-run gratuit, run interrompu reprenable. Les octets vont de Drive à Convex sans passer par une fonction (URL d'upload non authentifiée). Prérequis : `GDRIVE_TOKEN` (token Drive lecture, valable 1 h — OAuth Playground, scope `drive.readonly`). Exclut les documents nominatifs de tiers, certificats de signature, RIB, decks, >20 Mo, archives, et la version la plus légère d'un doublon. Runbook complet en tête du module. |
