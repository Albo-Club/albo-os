...
| Prévisionnel Airtable → échéances (org `calte`)                               | `convex/airtableImport.ts` → `airtableImport:forecastEntriesDryRun` / `forecastEntriesApply` / `forecastEntriesVerify` | Porte « Prévision de rentrée » / « Prévision de sortie » dans `forecastEntries` (ce que lit la page Cash) — l'import d'origine ne les écrivait que dans la table legacy `forecasts`, lue par rien. Re-tire les 2 tables **en direct** (Airtable est débranché : on lit au plus frais). Sens = table d'origine, jamais le signe du montant ; fiabilité `expected` (un prévisionnel n'est pas un engagement → courbe « avec planifié », courbe engagée intacte) ; lignes passées **importées** (elles sortent « en retard », les perdre embellirait la trajectoire en silence) ; 10 lignes qui redisent une règle récurrente exclues (`RULE_DUPLICATE_ROWS` dans `convex/lib/airtableForecasts.ts` — IROKO ×9, Wormser ×1) ; doublons internes à Airtable **signalés, jamais fusionnés** ; rattachement au deal seulement si la société Airtable résout vers **un seul** deal (le `1 deal = Entreprise × instrumentKind` d'origine a été défait depuis). Idempotent via `derivedKey` = `airtable:{recordId}` ; une échéance déjà rapprochée ou annulée à la main n'est jamais réécrite. Cœur pur testé (`tests/airtableForecasts.test.ts`). Runbook dans la section dédiée du module. |
...
| Import documentation juridique Albo Club (Drive → `documents`) | `convex/migrations/legalDocsImport.ts` → `dryRun` / `verify` + `node scripts/import-legal-docs.mjs` | Importe 320 documents sur 42 sociétés de l'org `albo` depuis le Drive « ⚠️ Investissements ». Correspondance figée et revue à la main dans `scripts/data/legal-docs-albo.json`. **Idempotent** sur `companyId`+`title`+`size` : re-run gratuit, run interrompu reprenable. ⚠️ Le rapport de doublons de `verify` utilise une clé **volontairement plus lâche** que ce garde-fou (société + titre normalisé, taille exclue) et sort des groupes `{ company, rows: [{ _id, title, size }] }` à arbitrer à la main — cf. `KNOWN_ISSUES.md` « Détecter un doublon et empêcher un doublon ». Les octets vont de Drive à Convex sans passer par une fonction (URL d'upload non authentifiée). Prérequis : `GDRIVE_TOKEN` (token Drive lecture, valable 1 h — OAuth Playground, scope `drive.readonly`). Exclut les documents nominatifs de tiers, certificats de signature, RIB, decks, >20 Mo, archives, et la version la plus légère d'un doublon. ⚠️ **À lancer seulement APRÈS le merge** : `convex run --prod` appelle le code déployé, et la prod se déploie par le build Vercel sur `main` — les fonctions n'existent pas en prod avant. Le merge est sans risque, rien ne s'exécute au déploiement. Runbook complet en tête du module. |
| Import documentation juridique CALTE (Drive → `documents`) | `convex/migrations/legalDocsImport.ts` → `dryRun` / `verify` + `node scripts/import-legal-docs.mjs --mapping scripts/data/legal-docs-calte.json` | Second lot du même import, org `calte` : **785 documents sur 190 fiches société** depuis le Drive « CALTE SAS › CALTE - Investissements » (958 fichiers inventoriés, 134 dossiers de participation). Correspondance figée et revue à la main dans `scripts/data/legal-docs-calte.json`. Le module Convex est **inchangé** (il était déjà générique : `dryRun`/`verify` prennent un `orgSlug`, `attachBatch` déduit l'org de la société) ; seul le script apprend `--mapping`. **Idempotent** sur `companyId`+`title`+`size`, mêmes garanties que le lot Albo. Exclut 128 fichiers (Google natifs, tableurs, images, archives, RIB, récapitulatifs Docusign, KYC/W-8, documents nominatifs de tiers) et **45 fichiers écartés sur arbitrage** (07/09/2026), en deux temps : 25 dont le dossier Drive ne correspond à aucune fiche société évidente (attestations de valorisation des SPV Anaxago, Banco, Heritage Biarritz, Omnis Liber, Pulse 1, Bastille Cycles) — les cinq fichiers SIDE de cette liste ayant été rattachés à la main à SIDE 1 / SIDE 2 via `ID_OVERRIDES` ; et 20 issus de cinq dossiers dont le rattachement restait ambigu, écartés via `SET_ASIDE` (Chapelle et Banco 2, **entités du groupe** et non participations ; Club Tylia → Sezame Immo 4 ; MARBLE → Good Tech Lab ; Obligation PIXEL - 18b Villiers → SCCV de la Main Jaune). Ces cinq-là restent à trancher : les rattacher plus tard = un second lot, pas un re-run. Prérequis et exécution identiques au lot Albo (`GDRIVE_TOKEN`, **lancer seulement APRÈS le merge**). Runbook en tête du module. |
| Relance des lectures de documents en échec | `convex/migrations/retryDocOcr.ts` → `run` | Remet en lecture les `documents` dont l'extraction s'est soldée par un échec, **espacées** de quelques secondes. Motif : `convex/lib/ocr.ts` renvoie `''` sur **toute** réponse non-OK de l'API Mistral OCR — un fichier illisible et une requête refusée pour cause de débit sont enregistrés à l'identique (`ocrState: 'failed'` / `ocrDetail: 'ocr_failed'`). Un import en masse laisse donc des documents marqués illisibles qui ne le sont pas : le lot juridique CALTE en a produit 171 sur 785. Rien ne les rattrape (le cron `sweepStalePending` ne reprend que les `pending`) et `documents:reextract` est derrière `requireOrgMember`, donc hors de portée de `convex run`. **Idempotent et rejouable** : ne prend que les lignes actuellement en `failed`, donc un re-run saute ce qui est reparti ; relancer jusqu'à `relaunched: 0`. Ce qui échoue encore après deux passes est vraiment illisible — `legalDocsImport:verify` les nomme. Args : `orgSlug` (requis), `limit` (défaut 250), `spacingSeconds` (défaut 3). Générique : marche pour n'importe quelle org. ⚠️ **À lancer seulement APRÈS le merge** (`convex run --prod` appelle le code déployé). Runbook en tête du module. |
| Suppression des doublons arbitrés du lot juridique CALTE | `convex/migrations/purgeDuplicateLegalDocs.ts` → `dryRun` / `apply` | Supprime les **15 documents en double** de l'org `calte` arbitrés à la main le 09/09/2026 depuis le rapport de `legalDocsImport:verify` (extraits de texte ajoutés par la PR #452) : 3 dépôts strictement identiques (dont deux partageant leur Docusign Envelope ID), 4 réexports plus légers du même bulletin signé (on garde le plus lourd), 8 scans dont un exemplaire n'a aucun texte lisible (on garde le lisible). Les paires sont **figées en dur par `_id` de prod**, jamais sous forme de règle : « garder le plus lourd » aurait détruit le bulletin BELLEVILLES rempli par CALTE (318 Ko) au profit du modèle vierge de l'émetteur (1,5 Mo) — BELLEVILLES et les 3 exports JEEN sont volontairement **hors liste**. Garde-fous : le survivant doit exister, les deux lignes être dans `calte`, sur la même société, avec la même clé `normalizeDocumentTitle` — sinon refus. **Idempotent** : une ligne déjà partie est signalée, pas une erreur. Le blob n'est libéré que par `releaseStorage` (compte de références), et l'entrée d'index sémantique est retirée via `vectorize:removeEntry`. ⚠️ **À lancer seulement APRÈS le merge** ; `dryRun` d'abord, relire les 15 lignes nommées, puis `apply`. Runbook en tête du module. |
| Backfill deals & sociétés depuis la doc juridique (org `albo`) | `convex/migrations/alboDocBackfill.ts` → `listTargets` / `getDocText` / `extractDocument` / `planForDeal` / `applyRows` + `node scripts/backfill-deal-fields.mjs` | Remplit les champs vides des `companies`/`deals` de l'org `albo` (jamais `calte`) à partir des pactes, bulletins, PV/statuts et term sheets déjà versés. **Extraction par LLM**, un appel par document, sortie JSON à schéma imposé : toute valeur sans **extrait verbatim** retrouvé littéralement dans le texte est rejetée. L'arbitrage (hiérarchie des sources PV > bulletin > pacte > term sheet _jamais autoritatif_ > BP _jamais source_), les conversions (centimes / bps / ISO) et les dérivations (post-money = base FD × prix du tour, pre-money = post − taille du tour) sont du code pur et testé (`convex/lib/docBackfill.ts`, cas de référence Auxicare dans `convex/regression.docBackfill.test.ts`). **Dry-run par défaut, zéro write** : produit un rapport markdown + un CSV en trois sections (PROPOSITIONS / ÉCARTS / NON TRAITÉ). Le CSV est le fichier d'entrée de l'apply : mettre `1` en colonne `ok` sur les lignes validées. **Rejouable sur le delta** (cache par documentId + hash du texte, court-circuite le modèle) et **idempotent** : `applyRows` n'écrit que si la colonne porte encore exactement ce que le dry-run a vu, donc un 2ᵉ apply est un no-op et une valeur non nulle n'est jamais écrasée en silence. ⚠️ **À lancer seulement APRÈS le merge** (`convex run --prod` appelle le code déployé). Runbook en tête du module ; pièges dans `KNOWN_ISSUES.md` « Backfill depuis la doc juridique ». |
...
| Fiches société orphelines de l'org `calte` | `convex/migrations/cleanupCalteOrphanCompanies.ts` → `dryRun` / `apply` | À lancer APRÈS `cleanupCalteImport`. Traite les 44 fiches `portfolio` restées sans aucun deal : **archive 41 fiches** qui ne sont pas des participations (dons et associations, prestataires facturant avec TVA, DGFIP, libellés bancaires et comptes de passage, artefacts d'import, personnes physiques, la fiche CALTE elle-même, une fiche de dealflow jamais investie et une participation de l'autre véhicule) — chaque ligne porte son `kind` comme preuve du classement, arbitré sur les mouvements et sur la plaquette signée au 31/12/2025. **Crée 3 deals manquants** : `PRIV. EQUITY ROTHSCHILD` (387 321 €, `fund_lp`, sans date — aucun mouvement en base) et `INVEST FOR PLANET` (5 000 €, `share`, 11/01/2021) révélés par le bilan, plus `LE CHAPTAL` (10 000 €, `cca`, 01/10/2025) lu sur les relevés et créé directement `fully_exited` (remboursé le 15/07/2026, `exitProceeds` 10 000 €). Les 3 mouvements déclarés sont pointés sous garde (org, montant, date, sens, non déjà pointé) — un mouvement qui a bougé depuis l'audit est remonté dans `skipped`, jamais forcé. ⚠ Deux archivages à ne pas mal lire : `Clement ALTERESCO` est un compte courant d'associé au **passif** (70 700 €) et `NANTISSEMENT CASH DEPOSIT PRET B` le gage espèces de 3 280 000 € — des réalités, mais pas des participations. **Idempotente** : ancrage par `_id` + garde sur le nom exact, archivage refusé s'il reste une référence (14 tables), deal non recréé si la fiche en porte déjà un. Runbook en tête du module. |
| Nettoyage du portefeuille CALTE issu de l'import Airtable | `convex/migrations/cleanupCalteImport.ts` → `dryRun` / `apply` / `verify` | Reprend les défauts de construction de l'import (`1 deal = Entreprise × instrumentKind`, cf. `convex/airtableImport.ts`), audités deal par deal sur leurs mouvements et recoupés avec la note Drive « Calte — remise au propre des données » (04/08/2026). `apply` enchaîne 7 blocs : renommage `Sant Roch` → `Sant Roch - Contrast` ; dates de signature manquantes (Parallel SPV24 / SPV25) ; création de `RM Expansion` + son deal actions (27 995,82 €, 03/07/2026) ; **fusion de 10 fiches société en double** (les 6 entités du groupe dupliquées en `portfolio` par l'import + Cœur Pigalle, Asterion Side Onima, Flexliving/Flex Living) et archivage de 2 coquilles Batch vides (⚠ les 4 fonds Batch, eux, sont légitimes) ; **fusion de 3 paires de deals** (Sezame Immo 1, C/C Flexliving, C/C Bureaux à Partager) ; **sortie du portefeuille de 3 lignes qui ne sont pas des invests** (prêt Wormser remboursé par CALTE, retrait de cagnotte Anaxago, cession de 30 titres B) — leurs mouvements repartent en file de pointage (`matchStatus: 'unmatched'`) ; **découpage de 27 deals agrégeant plusieurs opérations** en 62 (le deal existant garde la première opération et son `airtableId`, les suivantes deviennent des deals nommés via `deals.name`). Les mouvements sont appariés par (date, montant) dans le deal et consommés une fois : une part introuvable ou ambiguë fait sauter le deal entier plutôt qu'un découpage partiel. Les encaissements ne sont **jamais** redistribués (un exit porte sur l'ensemble des titres). **Idempotente** : ancrage par `_id` prod + garde sur le nom exact ; les fiches sont archivées (soft delete, refusé s'il reste une référence), les deals vidés sont supprimés une fois toutes leurs références repointées (invariant de `deals.remove`). Clés écrites ajoutées à `manuallyEditedFields`. ⚠️ Snapshot obligatoire avant `apply`. Hors périmètre : le dossier Bureaux à Partager (répartition titres / C/C à trancher avec Clément) et les 36 transactions à pointer à la main. Runbook en tête du module. |
...
| Effacement du « versé » figé du C/C Albo Club (org `calte`) | `convex/migrations/clearAlboCcaPaidAmount.ts` → `inspect` / `apply` | Retire `paidAmount` (400 000 €) sur l'unique deal `cca` de CALTE visant ALBO CLUB, où 1 880 000 € sont réellement pointés. **Le garde-fou est l'essentiel** : l'effacement n'a lieu que si `paidActual` dépasse `paidAmount`. Sur les 13 deals `calte` dont saisi ≠ pointé, 7 n'ont aucune transaction pointée — là, l'instantané est la seule information existante et l'effacer détruirait la donnée. Périmètre volontairement limité à cette ligne ; les trois autres deals dont le pointage est en avance n'ont pas été arbitrés. Idempotent : un second passage ressort `done`. |
...
2. **PR de suivi** (après purge effective) : retirer la table `forecasts` du
   `convex/schema.ts`, l'upsert `upsertForecasts` de l'import Airtable, et la
   section correspondante de `KNOWN_ISSUES.md`.

⚠️ Ne **pas** retirer au passage `fetchForecastSourceRows` (le tirage des
tables `prevRentree`/`prevSortie`) : depuis le port du prévisionnel vers les
échéances, il n'alimente plus seulement la table legacy — c'est aussi la
source de `airtableImport:forecastEntriesDryRun` / `forecastEntriesApply`.
Il ne devient supprimable qu'une fois Airtable définitivement débranché et
le port rejoué une dernière fois.
