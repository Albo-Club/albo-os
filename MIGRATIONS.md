...
| Audit de l'import Airtable (lecture seule)                           | `convex/migrations/auditAirtableImport.ts` → `report` + `scripts/render-audit-import.mjs`                    | Diagnostic, n'écrit rien. Liste les deals portant un `airtableId` avec leurs mouvements et cinq drapeaux (`to_split`, `intragroup`, `duplicate_target`, `pointing_gap`, `no_movement`), triés par montant. Le script rend le JSON en page HTML autonome pour la revue. Sert de file de travail avant toute correction du portefeuille importé.                                                                          |
