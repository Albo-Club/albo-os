 ...
 * - Attio bridge idempotence (`attioDealId` / `attioCompanyId`) comes from the
 *   sync upsert (keyed lookups on `by_attio_deal_id` / `by_attio_company_id`):
 *   re-running the same event never duplicates. The company anchor is NOT
 *   unique — one Attio record backs several entities — so the sync picks the
 *   oldest anchored company of the org rather than reading `.unique()`.
...
describe('Attio bridge: attioDealId / attioCompanyId idempotence', () => {
  ...
  test('several entities share the same anchor, the sync keeps its target', async () => {
    const t = setupHarness()
    const user = await createUser(t, 'albo@test.dev')
    ...
    // One Attio record ⟷ N entities (Attio knows one company where Albo OS
    // has one entity per SPV): claiming it a second time is allowed.
    const spv = await createPortfolioCompany(t, org.orgId, 'SPV 2')
    await user.as.mutation(api.companies.update, {
      id: spv,
      patch: { attioCompanyId: 'attio-company-1' },
    })

    // And the sync still routes to the oldest anchored company of the org.
    ...
    const { anchored, deal } = await t.run(async (ctx) => {
      const rows = await ctx.db.query('companies').collect()
      const deals = await ctx.db.query('deals').collect()
      return {
        anchored: rows.filter((c) => c.attioCompanyId === 'attio-company-1'),
        deal: deals[0],
      }
    })
    expect(anchored).toHaveLength(2)
    expect(deal.targetCompanyId).not.toBe(spv)
  })

  test('another org can claim the same anchor', async () => {
    const t = setupHarness()
    const user = await createUser(t, 'albo@test.dev')
    await createOrg(t, 'albo', [{ userId: user.userId, role: 'owner' }])
    const orgB = await createOrg(t, 'org-b', [
    ...
    // The anchor is one workspace-wide Attio record, which several orgs may
    // legitimately track — the sync scopes its lookup to the org.
  ...
  test('unlinking drops the anchor from that company', async () => {
    const t = setupHarness()
    const user = await createUser(t, 'albo@test.dev')
    const org = await createOrg(t, 'albo', [
      { userId: user.userId, role: 'owner' },
    ...
    // '' unlinks (the mutation drops the column), so the anchor moves entirely.
    await user.as.mutation(api.companies.update, {
      id: anchoredId,
