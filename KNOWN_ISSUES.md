...
1. `packageManager` dans `package.json` est **la** source de vérité. Les
   ...
   champ tout seul.
2. Le pin suit **ce que Vercel exécute déjà** (10.28.0 aujourd'hui). Sans
   `ENABLE_EXPERIMENTAL_COREPACK`, Vercel ignore `packageManager` : le champ
   est un no-op côté prod, il ne sert qu'à réaligner local et CI. Passer à
   pnpm 11 est donc un chantier à part, qui suppose d'activer le corepack
   expérimental sur Vercel — pas un simple bump de chaîne.
