...
| 2026-07-20 | External-connections core — registry + generic connection storage | `convex/lib/connectors.ts`, `convex/connections.ts`, `externalConnections` table (schema) (+ `tests/connectors.test.ts`) | Generic infra for any app talking to external platforms: declarative connector registry (scope / auth kind / required keys), one `externalConnections` table with secrets at rest, shared seed/remove/list/markConnected + a per-auth-kind `status` diagnostic. **Strip before upstream:** the Albo-specific registry entries (powens/vasco/notion/docsend) and the `connectionHealth` import from `powens.ts` — ship the core with an empty registry. | ⏳ flagged |
