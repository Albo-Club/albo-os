...
| ---------- | --------- | --------------- | ---------------- | ------ |
| 2026-07-30 | Dead-code detection (knip) — config + CI job | `knip.jsonc`, `deadcode` script in `package.json`, `deadcode` job in `.github/workflows/ci.yml` | Pure DX/CI tooling, project-agnostic. The config encodes the entry-point patterns any project on this stack needs (TanStack Start file-based routes, Convex functions behind the generated `api.*`/`internal.*` proxies, vendored dirs, CSS-only deps) — exactly the non-obvious part worth shipping with the template. **Strip before upstream:** Albo-specific ignores (`convex/migrations/*`), and flip the CI job to blocking since the template starts clean. | ⏳ flagged |
