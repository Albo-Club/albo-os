...
| 2026-07-21 | Convex auto-update workflow — weekly PR when a new convex version ships | `.github/workflows/update-convex.yml`, `scripts/update-convex.mjs` | Same pattern as `sync-skills.yml`: weekly cron bumps `convex@latest`, runs lint/tests/build in the workflow (auto-PRs made with `GITHUB_TOKEN` don't trigger CI), then opens a validated `chore/update-convex` PR. **Strip before upstream:** the changelog-entry generation (CHANGELOG_PRODUIT.md is Albo OS-specific). | ⏳ flagged |
